...
for c=1:size(N,2)
  ...
  %%%change made by kunal on 19 june
%   vEvT{c} = v(:,c_elements) .* repmat(1./E(c_elements), size(v, 1), 1) * v(:,c_elements)';
  vEvT{c} = v(:,c_elements) .* repmat(1./E(c_elements)', size(v, 1), 1) * v(:,c_elements)';

end

%change made bu kd on 19june 
% for ii = unique(spk_ids)'
for ii = unique(spk_ids)
    %following changes make by kunal on 19june
%   speakers_sessions = find(spk_ids == ii);
  speakers_sessions=ii;
  Fs = F(speakers_sessions,:);
%   F_un_normalised=Fs;
  Nss = N(speakers_sessions,:);
  Ns = Nss(1,index_map);
  %changes made y kd on 19 june
%   Fs = Fs -  (m + z(ii,:) .* d) .* Ns;
  Fs = Fs -  m' .* Ns;
...
%   cost1=cost1+(0.5*(F_un_normalised*inv(diag(E))*(v')*(y')));
%   cost2=cost2-(0.5*norm(log10(L),'fro'));
end  
...
% cost=cost1+cost2;
% disp(['The cost for this (actually previous) iteration is:',num2str(cost)]);
