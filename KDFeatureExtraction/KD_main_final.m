...
kd_extract_features_train(files,F_total,N_total,W_mean_supervector,W_covariance_supervector,T,dim_of_features,num_of_basis_elements,bpre,apre,win_len,win_shift,nfft,Wsparsity,Hsparsity,outdir,datadir);

%%%Step 4: Extracting the same for the test set
disp('on Step 4: Calculating the time-activations for the test set')
kd_extract_features_test(bpre,apre,W_ubm,Wsparsity,Hsparsity,dim_of_features,num_of_basis_elements,c_main,W_mean_supervector,W_covariance_supervector,T,outdir,datadir,win_len,win_shift,nfft);
